        ...
        if (recipients.length) {
            ...
            try {
                ...
                if (skippedUsers.length) {
                    logger.warn('notification_service.createAndDispatch: socket targets skipped due to missing user_id', {
                        notificationId: notification.id,
                        skippedUsers: skippedUsers.map((user) => ({ businessUserId: user.id, userId: user.user_id })),
                    })
                }
