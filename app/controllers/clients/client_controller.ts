...
export default class ClientController {
  ...
  public async index(ctx: HttpContext) {
    ...
    try {
      ...
      const clients = await (page ? clientsQuery.paginate(page, perPage) : clientsQuery)
    ...
    try {
      ...
      if (uploadFiles && uploadFiles.length) {
        ...
        for (const f of uploadFiles) {
          ...
          const { url, url_short } = await Google.uploadFile(
            f,
            'clients',
            isImage ? 'image' : 'file'
          )
  ...
  public async update(ctx: HttpContext) {
    ...
    const { clientDocumentInvoiceId, clientDocumentInvoiceValue, systemPaymentProvider, ...data } =
      await request.validateUsing(clientUpdateValidator)
    const dateTime = await Util.getDateTimes(request)
    const trx = await db.transaction()
    try {
      ...
      if (clientDocumentInvoiceId || clientDocumentInvoiceValue || systemPaymentProvider) {
        await client.related('documentInvoice').updateOrCreate(
          { clientId: clientId },
          {
            clientId,
            documentInvoiceId: clientDocumentInvoiceId,
            systemPaymentProvider,
            value: clientDocumentInvoiceValue,
          }
        )
      }
