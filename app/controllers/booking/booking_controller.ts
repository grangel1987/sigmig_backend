...
export default class BookingController {
    ...
    public async store({ request, response, i18n }: HttpContext) {
        ...
        try {
            ...
            const b: any = { ...booking }
            if (b?.type === 'MENSUAL') {
                const checkOut = Util.getDateAddMonths(DateTime.fromISO(b.checkIn), b.monthQuantity || 0, false)
                b.checkOut = checkOut
            } else {
                b.monthQuantity = 0
            }
            ...
            const bk = await Booking.create({
                ...
                createdAt: dateTime,
                checkIn: DateTime.fromISO(b.checkIn),
                checkOut: DateTime.fromISO(b.checkOut),
                type: b.type || '',
            ...
            const guestsRows = (guests as any[]).map((g) => ({
                name: g?.name || '',
                lastName: g?.lastName ?? '',
                identifyTypeId: g?.identifyTypeId,
                identify: g?.identify || '',
                phone: g?.phone || '',
                email: g?.email || '',
                fromWhere: g?.fromWhere ?? '',
                answer1: !!g?.answer1,
                answer2: !!g?.answer2,
                mobilityPassUrlShort: g?.mobilityPassUrlShort ?? null,
                mobilityPassUrl: g?.mobilityPassUrl ?? null,
            }))
            if (guestsRows.length) await bk.related('guests').createMany(guestsRows as any)

            // Property via Lucid
            const propId = (propertie as any).propertieId
            if (propId) {
            ...
            const itemsRows = (items as any[]).map((it) => ({
                itemId: (it as any)?.itemId ?? (it as any)?.value,
            ...
            const payloadEmail: any = {
                propertie: (propertie as any)?.propertieView,
                items,
                feedings: feeding,
                guests,
                email: undefined,
