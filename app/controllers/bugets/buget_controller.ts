...
import {
  ...
  bugetFindByDateValidator,
  bugetFindByNameClientValidator,
  bugetFindByNroValidator,
  bugetObservationValidator,
...
export default class BugetController {
  public async store(ctx: HttpContext) {
    ...
    try {
      ...
      const banksRows = (banks as any[]).map((b) => ({
  ...
  public async show(ctx: HttpContext) {
    ...
    if (expireDate) {
      ...
      serialized.expire_date_format = Util.parseToMoment(expireDate, false, {
        separator: '/',
        firstYear: false,
      })
    } else {
  ...
  public async showPublic(ctx: HttpContext) {
    ...
    if (expireDate) {
      ...
      serialized.expireDateFormat = Util.parseToMoment(expireDate, false, {
        separator: '/',
        firstYear: false,
      })
    } else {
  ...
  public async findByNameClient(ctx: HttpContext) {
    ...
    const { request } = ctx
    const { businessId, name, page, perPage, status, budgetStatus } = await request.validateUsing(
      bugetFindByNameClientValidator
    )
    return await BugetRepository.findByNameClient(
      Number(businessId),
      String(name || ''),
      page,
      perPage,
  ...
  public async findByDate(ctx: HttpContext) {
    ...
    const { request } = ctx
    const { businessId, date, page, perPage, status, budgetStatus } =
      await request.validateUsing(bugetFindByDateValidator)
    const dateSql = DateTime.fromJSDate(date).toSQLDate()!

    return await BugetRepository.findByDate(
      Number(businessId),
      String(dateSql),
      page,
      perPage,
  ...
  public async update(ctx: HttpContext) {
    ...
    try {
      ...
      const business = await Business.query({ client: trx })
        .where('id', existingBuget.businessId!)
        .firstOrFail()
      const daysExpire = business.daysExpireBuget || 0
      const expireDateISO = Util.getDateAddDays(dateTime, daysExpire)
      const expireDate = DateTime.fromISO(expireDateISO)
 ...
 */
  ...
  public async sendEmailToClient(ctx: HttpContext) {
    ...
    try {
      ...
      const expirationDate = buget.expireDate
      ...
      const body = i18n.formatMessage('messages.budget_email_body', {
        clientName,
        budgetNumber,
        expirationDate,
        businessName,
...
export async function sendBudgetNotification(
  ...
  const superUsers = await BusinessUser.query()
    ...
    .preload('user', (userQuery) => {
      ...
      userQuery.preload('personalData', (pdQ) =>
